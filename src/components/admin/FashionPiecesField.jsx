...
export default function FashionPiecesField({ value, onChange, token, talentOptions, lookTitle }) {
    ...
    <div className="admin-fashion-pieces-field">
      {pieces.map((piece, index) => (
        <div key={index} className="admin-fashion-piece-card">
          ...
          <div>
            <label className="admin-modal-label admin-fashion-piece-card-credits-label">
              Credit overrides (leave empty to use the Look's default credits)
            </label>
            <CreditsField
              value={piece.credits}
      ...
      <button type="button" onClick={addPiece} className="admin-artists-page-primary-btn">
        Add Piece
      </button>
    </div>
  )
