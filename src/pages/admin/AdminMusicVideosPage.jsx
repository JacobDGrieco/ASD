...
function rowToForm(row) {
	return {
		...
		videoExtension: getStaticArtistVideoExtension(row.videoUrl),
...
function buildArtistStaticVideoUrl(rows, artistId, currentVideoUrl = '', baseUrl = '', selectedExtension = '') {
	const artist = findArtist(rows, artistId);
	const extension = selectedExtension || getStaticArtistVideoExtension(currentVideoUrl);
...
export default function AdminMusicVideosPage() {
	...
	const handleSave = async () => {
		const payload = {
			...buildPayload(form),
			videoUrl: form.sourceType === ARTIST_VIDEO_SOURCE.UPLOAD ? buildArtistStaticVideoUrl(rows, form.artistId, form.videoUrl, VIDEO_BASE_URL, form.videoExtension) : form.videoUrl,
		};
								...
								<div className="admin-modal-field admin-modal-field-full">
									...
									<select
										...
										onChange={(event) => setForm((current) => ({
											...
											videoUrl: event.target.value === ARTIST_VIDEO_SOURCE.UPLOAD ? buildArtistStaticVideoUrl(rows, current.artistId, current.videoUrl, VIDEO_BASE_URL, current.videoExtension) ?? '' : '',
										}))}
								...
								{form.sourceType === ARTIST_VIDEO_SOURCE.UPLOAD && (
									<div className="admin-modal-field">
										...
										<select
											...
											onChange={(event) => setForm((current) => ({
												...current,
												videoExtension: event.target.value,
												videoUrl: buildArtistStaticVideoUrl(rows, current.artistId, current.videoUrl, VIDEO_BASE_URL, event.target.value) ?? '',
											}))}
											className="admin-artists-page-input"
										>
